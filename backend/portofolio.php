...
<head>
	...
	<title>Limitless - Portofolio</title>
					...
					<ul class="nav nav-sidebar" data-nav-type="accordion">
						...
						<li class="nav-item nav-item-submenu nav-item-open nav-item-expanded">
							...
							<ul class="nav nav-group-sub" data-submenu-title="Page layouts">
								...
								<li class="nav-item"><a href="portofolio.php" class="nav-link active">Portofolio</a></li>
					...
					<ul class="navbar-nav flex-row">
						...
						<li class="nav-item nav-item-dropdown-lg dropdown dropdown-user">
							<a href="#" class="navbar-nav-link navbar-nav-link-toggler dropdown-toggle" data-toggle="dropdown">
								...
								<span class="d-none d-lg-inline-block">Hanna Dorman</span>
							</a>
				...
				<div class="page-header">
					<div class="page-header-content d-sm-flex">
						<div class="page-title">
							<h4><span class="font-weight-semibold">Page Testimoni</h4>
						</div>
					</div>
				</div>
				<!-- /page header -->
									...
									<div class="card-body">
										...
										if (isset($_POST['submit'])) {
											...
											if (!in_array($ext1, $ekstensi)) {
												echo "<script>
			 											alert('data gagal di tambahkan, ekstensi salah');
			  											document.location.href = 'portofolio.php';
			 										</script>";
											} else {
												if ($ukuran1 < 1044070) {
													$galeri = $rand . '_' . $filename1;
													move_uploaded_file($_FILES['foto1']['tmp_name'], 'global_assets/portofolio/' . $rand . '_' . $filename1);
												} else {
													echo "<script>
															 alert('data gagal di tambahkan, file terlalu besar');
															  document.location.href = 'portofolio.php';
														 </script>";
												}
											}

											if (!in_array($ext2, $ekstensi)) {
												echo "<script>
														alert('data gagal di tambahkan, ekstensi salah');
												 		document.location.href = 'portofolio.php';
													</script>";
												...
												if ($ukuran2 < 1044070) {
													$klien = $rand . '_' . $filename2;
													move_uploaded_file($_FILES['foto2']['tmp_name'], 'global_assets/portofolio/' . $rand . '_' . $filename2);
													mysqli_query($koneksi, "INSERT INTO portofolio VALUES(NULL,'$judul','$galeri','$klien')");
													if (mysqli_affected_rows($koneksi) > 0) {
														echo "<script>alert('data berhasil di tambahkan');
																	document.location.href = 'portofolio.php';</script>";
													} else {
														echo "<script>alert('data gagal di tambahkan');
																	document.location.href = 'portofolio.php';</script>";
													}
												} else {
													echo "<script> alert('data gagal di tambahkan, file terlalu besar');
														document.location.href = 'portofolio.php';</script>";
												}
											}
										}
										...
										<form action="" method="POST" enctype="multipart/form-data">
											...
											<div class="form-group">
												...
												<input type="file" name="foto1" class="input" required></i></input>
											</div>
											<div class="form-group">
												<label>Gambar Logo Klien</label>
												<input type="file" name="foto2" class="input" required></i></input>
											</div>
											<br>
										...
										<tbody>
											...
											?>
												<tr>
													...
													<td><img src="global_assets/portofolio/<?= $data['gambar_galeri']; ?>" width="130" height="140"> </td>
													<td><img src="global_assets/portofolio/<?= $data['gambar_klien']; ?>" width="130" height="140"> </td>
												</tr>
												<?php $no++ ?>
											<?php } ?>
										</tbody>
